...
func NewPlateSolver(
	...
	go func() {
		...
		stars = sexp.FindStars(stats, sigma, 2.3)
...
func (ps *PlateSolver) MatchAsterismsWithCatalog(
	...
	for _, perm := range permutations {
		...
		x1, y1 := sources[i0].RA, sources[i0].Dec
		x2, y2 := sources[i1].RA, sources[i1].Dec
		x3, y3 := sources[i2].RA, sources[i2].Dec
...
func (ps *PlateSolver) FindSourceMatches(tolerance geometry.InvariantFeatureTolerance) ([]Match, error) {
	...
	for _, asterism := range asterisms {
		go func(asterism astrometry.Asterism) {
			defer wg.Done()
			...
			for _, source := range sourceAsterisms {
				if geometry.CompareInvariantFeatures(asterism.Features, source.Features, tolerance) {
...
func (ps *PlateSolver) Solve(tolerance geometry.InvariantFeatureTolerance, sipOrder int) (*wcs.WCS, []Match, error) {
	...
	if len(matches) < 3 {
		return nil, nil, errors.New("insufficient matches to perform plate solving")
	}
	...
	if affineParams == nil {
		return nil, nil, errors.New("failed to compute affine transformation matrix parameters")
	}
	...
	if fsipParams == nil {
		return nil, nil, errors.New("failed to compute SIP transformation matrix parameters")
	}
	...
	if isipParams == nil {
		return nil, nil, errors.New("failed to compute inverse SIP transformation matrix parameters")
	}
